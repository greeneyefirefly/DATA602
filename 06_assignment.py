...
def exercise09():
    ...
    # Creating and tidying the dataframe
    df = pd.DataFrame(eval(requests.get(url, headers=headers).content)).drop(
            index = ['updated','updatedISO'], columns = ['disclaimer', 
                    'time']).reset_index().rename(columns = {"index":"time"})
    print(df.head())
    print(df.tail())
    
    # Plot and publish price timeseries to Plotly
    import plotly.graph_objects as go
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x = df.time, y = df.bpi, name = "Bitcoin Price Index", line_color='deepskyblue'))

    fig.update_layout(title = 'Bitcoin Price Index Timeseries from 2017-09-01 to 2019-10-05', 
                      xaxis_title = "Date", yaxis_title = "Bitcoin Price Index", 
                      xaxis_rangeslider_visible = True)
    
    plotly_url = cs.plotly.plot(fig, filename = 'Bitcoin Price Index Timeseries')
        
    # Under my credentials, the plotly_url is https://plot.ly/~greeneyefirefly/3/
...
def exercise12(n):
    ...
    checkerboard_matrix = np.matrix(([1,0]*(n) + [0,1]*(n))*(n)).reshape(((2*n), (2*n)))
...
def exercise13(n):
    ...
    cumulative sum on a chart. The date range should be an index.
    
    '''
    # ------ Place code below here \/ \/ \/ ------
    
    s = pd.Series(np.random.randint(1, n, size = n), index = pd.date_range(start='1/1/2010', periods = n))
...
def exercise14(words):
    ...
    df = pd.DataFrame(words)[0].map(len)
