...
2. Block - A block holds a pool of transactions in a DataFrame. The maximum a single block can hold is 10 transactions. When a block is created, it contains zero transactions and has a status of UNCOMITTED. Once a block contains 10 transactions, that block then is marked COMMITTED and a new block is created for future transactions. Blocks are chained together by their block hash ID and previous block hash. Each block, except the first genesis block, tracks the hash of the previous block. When a block generates its own hash identifier, it uses the previous blocks hash as one of several strings it will concantenate. 
A block consists of:
    ...
    Merkle trie (yes, that's spelled trie!) uses a binary tree. We won't do that here. In our case, we will not use a tree but simply take the hash of the string concatenation of all the transaction hashes in a block once a block is full (reaches 10 transactions)
    
    - Block hash: The hash of this block is created by the hash of the string concatenation of the previous block's hash, the chains hash id, current date time, sequence id of the block and the root Merkle hash. The block hash is generated when a block is full and is committed.
    
3. PandasChain - A container class that manages all interaction to the internal state of the chain, i.e. users only interact with an instance of PandasChain and no other class. A PandasChain class consists of:
    - Name: An arbitrary name of this instance of the chain provided in the constructor when PandasChain is created (see test cases for usage examples)
    
    - Chain: A Python list of blocks
    
    - Chain ID: A hash concatenation of a UUID, name of the chain, timestamp of creation of the chain that uniquely identifies this chain instance.
...
The only way to interact with a PandasChain instance is via the add_transaction() method that accepts new transactions and methods that print out chain data like display_block_headers(). There should be no other way to reach the underlying blocks or pandas DataFrames that hold transactions.
...
To Do
-------------
One way to tackle this assigment is to start with these classes blank and slowly build it out in a separate file so that as you build it out it continues to run without errors, allowing you to incrementally build and test. There is a parent/child one-to-many relationship between PandasChain, Blocks and the transactions a block holds. PandasChain is parent to
one or more blocks and manages the lifecycle of those blocks. Blocks manage transactions.
To break this into manageable chunks, in an isolated environment create a pandas table with the columns listed below in the code and make sure you how to append rows to it. Then wrap it in a Block class and make the Block class work. Then build out the PandasChain class.
...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__id = hashlib.sha256(str(str(uuid.uuid4()) + self.__name +            
                                       str(dt.datetime.now())).encode('utf-8')).hexdigest()
    ...
    # It is private and therefore not public accessible. It will change the block status to committed, obtain the merkle root hash, 
    # generate and set the block's hash, set the prev_hash to the previous block's hash, append this block to the chain list, 
    # increment the seq_id and create a new block as the current block
    def __commit_block(self, block): 
        # Add code here
        block.set_status("COMMITTED") # change block status to committed
        self.__block_hash = hashlib.sha256()
        self.__block_hash.update(('{}{}{}{}{}'.format(self.__prev_hash, self.__id, str(
            dt.datetime.now()), self.__seq_id, block.get_simple_merkle_root())).encode('utf-8')) # obtain the merkle hash
        self.__block_hash = self.__block_hash.hexdigest()
        block.set_block_hash(self.__block_hash) # set block's hash
        self.__prev_hash = self.__block_hash  # set prev_hash to previous block hash
        self.__chain.append(self.__current_block) # append block to chain list
        self.__seq_id = self.__seq_id + 1 # increment sequence ID
        self.__current_block = Block(self.__seq_id, self.__prev_hash) # new block as the current block
        # Add code here
        print('Block Committed')
    
    # 10 pts - Display just the metadata of all blocks (committed or uncommitted), one block per line. You'll display the sequence Id, 
    # status, block hash, previous block's hash, merkle hash and total number (count) of transactions in the block
    ...
    def get_values(self):
        ...
        for block in self.__chain:
            block_value, timestamp = block.get_values()
            for i in block_value:
                transaction_list.append(i)
            for i in timestamp:
                timestamp_list.append(i)
                
        current_block_value, current_timestamp = self.__current_block.get_values()       
        for i in current_block_value:
            transaction_list.append(i)
        for i in current_timestamp:
            timestamp_list.append(i)
            
        # Plot #1 Time Series of Transaction
        pd.plotting._converter.register()
        fig = plt.figure()        
        plot1 = fig.add_subplot(111)
        plot1.plot(timestamp_list, transaction_list)
        plot1.set(title = 'Time Series of Transaction',
                  xlabel = 'Timestamp',
                  ylabel = 'Transaction Value')                   
        
        return [transaction_list, timestamp_list], fig 

# Exercise #2. Using PandasChain's get_values(), get all of the transaction values across all blocks in the chain 
# and plot them. Use an incrementing sequential number for x and the transaction values for y.
     
def exercise02(blockchain):
    # Plot #2 of all transaction values across all blocks in the chain 
    transaction_list, timestamp_list = blockchain.get_values()[0]
    plt.close()
    fig = plt.figure()        
    plot2 = fig.add_subplot(111)
    plot2.plot(range(0, len(transaction_list)), transaction_list) 
    plot2.set(title = 'All Transaction Values Across All Blocks in the Chain', 
              ylabel ='Transaction Value')   
    return fig      
...
class Block:
    ...
    # 5 pts -  Display on a single line the metadata of this block. You'll display the sequence Id, status, block hash, previous block's 
    # hash, merkle hash and number of transactions in the block
    def display_header(self): 
        print('Sequence ID: {}  Block Status: {}  Block Hash: {}  Previous Hash: {}  Merkle Hash: {}  Block Size: {}'.format(self.__seq_id, self.__status, self.__block_hash, self.__prev_hash, self.get_simple_merkle_root(), self.__transactions.shape[0]))
    ...
    # 10 pts - Return and calculate merkle hash by taking all transaction hashes, concatenate them into one string and hash that 
    # string producing a "merkle root" - Note, this is not how merkle tries work but is instructive and indicative in terms of the 
    # intent and purpose of merkle tries
    def get_simple_merkle_root(self): 
        merkle_string = self.__transactions["TxHash"].sum() # Concatenate all transaction hashes into one string
        merkle_root = hashlib.sha256() # Hash string to produce a merkle root
        merkle_root.update(('{}'.format(merkle_string)).encode('utf-8'))
        merkle_root = merkle_root.hexdigest() 
        self.__merkle_tx_hash = merkle_root # Merkle hash
...
2. Kansal, Satwik. Develop a blockchain application from scratch in Python. 
    https://www.ibm.com/developerworks/cloud/library/cl-develop-blockchain-app-in-python/index.html
3. Computer Networks and Security. Building tiny block-chain with python. 
    https://www.youtube.com/watch?v=4q8cYgsbSFE
4. Python Tutorial for Beginners - Learn Python by Building a Blockchain & Cryptocurrency. 
    CUNY SPS Data 602 course. Fall 2019.
5. Consensus Algorithms, Blockchain Technology and Bitcoin UCL - by Andreas M. Antonopoulos. 
    CUNY SPS Data 602 course. Fall 2019.
6. Creatively Grateful. Merkle Tree Hashing Algorithm Implementation In Python. 
...
class TestAssignment4(unittest.TestCase):
    def test_chain(self):
        ...
        exercise02(pandas_chain) # Exercise 02 test harness added for execution
